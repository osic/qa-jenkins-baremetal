...
def aggregate_parse_failed_smoke(host_ip, results_file, elasticsearch_ip) {
	...
	if (results_file == 'after_upgrade') {
	    ...
	    sh """
            ssh -o StrictHostKeyChecking=no ubuntu@${elasticsearch_ip} '''
	    elastic-upgrade -u \$HOME/output/api.uptime.out -d \$HOME/output/during.uptime.out -p \$HOME/output/persistent_resource.txt -b \$HOME/subunit/smoke/before_upgrade -a \$HOME/subunit/smoke/after_upgrade -g \$HOME/output/swift_api_status.json -w \$HOME/output/nova_api_status.json
